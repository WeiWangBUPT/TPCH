...
def Hash(pk1,cpabe,pk,ct_msg, h_msg, policy_str,key, sig_params, bls, secret_key): 
    ...
    # h1 = int.from_bytes(sha256(str(temp_hmsg).encode()).digest(), byteorder='big') * pow(r1, rsaKey.e, rsaKey.n) % rsaKey.n
    h2 = int.from_bytes(sha256(str(temp_hmsg).encode()).digest(), byteorder='big') * pow(r2, etd_rsaKey.e, etd_rsaKey.n) % etd_rsaKey.n
...
def Verify(pk1, etd_rsaKey, h_msg, r, h, bls, public_key, signature, ctxt):
    ...
    h1Prime = int.from_bytes(sha256(str(temp_hmsg).encode()).digest(), byteorder='big') * pow(r[0], pk1['e'], pk1['n']) % pk1['n']
    
    #h1Prime = int.from_bytes(sha256(str(temp_hmsg).encode()).digest(), byteorder='big') * pow(r[0], rsaKey.e, rsaKey.n) % rsaKey.n
    h2Prime = int.from_bytes(sha256(str(temp_hmsg).encode()).digest(), byteorder='big') * pow(r[1], etd_rsaKey.e, etd_rsaKey.n) % etd_rsaKey.n
...
def Adapt(param,pk1,TCH_r1_shares,sk_unshared,cpabe,pk, ctxt, aes_data, iv, h_msg, h_msgPrime, r, key, etd_rsaKey, h_rsa, policy_str, bls, secret_key):
    ...
    x1 = int.from_bytes(sha256(str(temp_hmsg).encode()).digest(), byteorder='big')
    x1Prime = int.from_bytes(sha256(str(temp_hmsgPrime).encode()).digest(), byteorder='big')
    y1 = x1 * pow(r[0], pk1['e'], pk1['n']) % pk1['n']
    x2 = int.from_bytes(sha256(str(temp_hmsg).encode()).digest(), byteorder='big')
    x2Prime = int.from_bytes(sha256(str(temp_hmsgPrime).encode()).digest(), byteorder='big')
