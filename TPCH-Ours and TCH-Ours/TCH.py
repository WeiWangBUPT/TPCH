...
def VV_Verify(pk1, h_msg, r, h):
    ...
    h1Prime = int.from_bytes(sha256(str(m).encode()).digest(), byteorder='big') * pow(r, pk1['e'], pk1['n']) % pk1['n']
...
def TCH_Hash(pk1,temp_hmsg):
    r1 = random.getrandbits(1024)
    h1 = int.from_bytes(sha256(str(temp_hmsg).encode()).digest(), byteorder='big') * pow(r1, pk1['e'],pk1['n']) % pk1['n']
    return h1, r1
